...
from src.ensembles.uniform_ensemble import UniformEnsemble
from src.models.utils import rename_model
...
def perform_experiment(surrogate_models, target_models, loader, lower_percentiles, upper_percentiles, filename, device=torch.device("cuda")):
    all_rows = []
    
    for s_model in surrogate_models:
        s_name = s_model.__class__.__name__

        for lp in lower_percentiles:
...
def main(config):
    ...
    print(f"Evaluating on {len(surrogates)} surrogates and {len(targets)} targets.")
    ...
    perform_experiment(
        ...
        loader=loader,
        lower_percentiles=config.lower_percentiles,
        upper_percentiles=config.upper_percentiles,
